async function scheduleHtmlProvider(iframeContent = "", frameContent = "", dom = document) {
  ...
  const userConfrim = await AIScheduleConfirm({
    ...
    contentText: "请在登录后进入可用应用-本科教务-我的课表-学生组后(即看到自己的课表后)再点击导入", // 提示信息，字体稍小，支持使用``达到换行效果，具体使用效果建议真机测试，为必传，不传显示版本号
  ...
  if (userConfrim) {
    ...
    try {
      ...
      searchParams.append('XNXQDM', XNXQDM)
      searchParams.append('SKZC', SKZC)
      //获取已经安排时间的课程
